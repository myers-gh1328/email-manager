...
describe('operator visibility contract', () => {
  ...
  test('keeps risky send actions and approval language off the dashboard', () => {
    ...
    expect(dashboard).not.toContain('Failed emails');
    expect(dashboard).not.toContain('Failed email history');
    expect(dashboard).not.toContain('href="/history?status=failed"');
    ...
    expect(dashboard).not.toContain('View failed sends in History before retrying from the scheduled email detail page.');
    expect(dashboard).not.toContain('email issue');
    expect(dashboard).not.toContain('failedTodayCount');
    expect(pageData).not.toContain('failedTodayCount');
    expect(pageData).not.toContain('countFailedCampaignDeliveriesBetween');
  ...
  test('keeps contact detail focused on recent email activity with a filtered History link', () => {
    ...
    expect(contacts).toContain('<p class="eyebrow">Contact record</p>');
    expect(contacts).not.toContain('<p class="eyebrow">Student record</p>');
  ...
  test('keeps class email as a visible primary action instead of a one-item collapse', () => {
    ...
    expect(classDetail).toContain('Will create one scheduled email for this class. Each student gets their own message.');
    expect(classDetail).toContain('<button type="submit">Create scheduled email</button>');
    expect(classDetail).not.toContain('Will create {form.previews.filter((preview) => !preview.skipped).length} scheduled emails, one to each student.');
    expect(classDetail).not.toContain('<button type="submit">Create scheduled emails</button>');
  ...
  test('uses plain scheduled-email language instead of approval campaign copy', () => {
    ...
    expect(scheduledEmails).toContain('href={`/scheduled-emails/${scheduledEmail.id}?returnTo=${encodeURIComponent(scheduledEmailReturnTo)}`}');
    ...
    expect(scheduledEmails).toContain('scheduledEmailsPage.status');
    expect(scheduledEmails).toContain('scheduledEmailsPageHref');
    expect(scheduledEmails).not.toContain('campaignsSearch');
    expect(scheduledEmails).not.toContain('campaignsStatus');
    expect(scheduledEmails).not.toContain('currentCampaignsPage');
    expect(scheduledEmails).not.toContain('totalCampaignsPages');
    expect(scheduledEmails).not.toContain('campaignsPageHref');
    expect(scheduledEmails).not.toContain('campaignWorkflowReturnTo');
    ...
    expect(scheduledEmails).toContain('<button type="submit">Preview scheduled email</button>');
    expect(scheduledEmails).not.toContain('Preview personalization');
    expect(scheduledEmails).toContain('<h3>Scheduled email preview</h3>');
    expect(scheduledEmails).toContain('Will create one scheduled email for the selected class. Each student gets their own message.');
    expect(scheduledEmails).toContain('Missing template fields: {preview.missing.join');
    expect(scheduledEmails).toContain('Skipped because: {preview.reason}');
    expect(scheduledEmails).not.toContain('Missing: {preview.missing.join');
    expect(scheduledEmails).not.toContain('Skipped: {preview.reason}');
    expect(scheduledEmails).toContain('<h3>Create scheduled email</h3>');
    expect(scheduledEmails).not.toContain('<h3>Choose send time</h3>');
    ...
    expect(scheduledEmails).toContain('{#if scheduledEmailsSearch}<input name="search" type="hidden" value={scheduledEmailsSearch} />{/if}');
    expect(scheduledEmails).toContain('{#if scheduledEmailsStatus}<input name="status" type="hidden" value={scheduledEmailsStatus} />{/if}');
    expect(scheduledEmails).toContain('{#if currentScheduledEmailsPage > 1}<input name="page" type="hidden" value={currentScheduledEmailsPage} />{/if}');
    expect(scheduledEmails).toContain('Page {currentScheduledEmailsPage} of {totalScheduledEmailsPages}');
