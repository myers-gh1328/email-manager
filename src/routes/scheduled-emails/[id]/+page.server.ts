...
export const actions = {
  ...
  deleteCampaign: async ({ params, request }) => {
    ...
    throw redirect(303, localReturnTo(formText(form.get('returnTo'))) || '/scheduled-emails');
  },
...
function detailActionReturn(campaignId: string, form: FormData, message: string) {
  ...
  return query ? `/scheduled-emails/${campaignId}?${query}` : `/scheduled-emails/${campaignId}`;
}
